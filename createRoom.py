...
import pickle
import select
from python_chat_server import chat1
...
def server():
    ...
    # Enviar el turno inicial al cliente
    client_socket.sendall(pickle.dumps(turn))
    
    chat_thread = threading.Thread(target=chat1)
    chat_thread.start()
